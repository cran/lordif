...
for (i in 1:ni) {
cutoff[i,1]<-getcutoff(chi12[,i],alpha,FALSE)
cutoff[i,2]<-getcutoff(chi13[,i],alpha,FALSE)
cutoff[i,3]<-getcutoff(chi23[,i],alpha,FALSE)
cutoff[i,4]<-getcutoff(pseudo12.CoxSnell[,i],alpha,TRUE)
cutoff[i,5]<-getcutoff(pseudo13.CoxSnell[,i],alpha,TRUE)
cutoff[i,6]<-getcutoff(pseudo23.CoxSnell[,i],alpha,TRUE)
cutoff[i,7]<-getcutoff(pseudo12.Nagelkerke[,i],alpha,TRUE)
cutoff[i,8]<-getcutoff(pseudo13.Nagelkerke[,i],alpha,TRUE)
cutoff[i,9]<-getcutoff(pseudo23.Nagelkerke[,i],alpha,TRUE)
cutoff[i,10]<-getcutoff(pseudo12.McFadden[,i],alpha,TRUE)
cutoff[i,11]<-getcutoff(pseudo13.McFadden[,i],alpha,TRUE)
cutoff[i,12]<-getcutoff(pseudo23.McFadden[,i],alpha,TRUE)
cutoff[i,13]<-getcutoff(beta12[,i],alpha,TRUE)
}
