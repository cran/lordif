plot.lordif <-
function(x,labels=c("Reference","Focal"),...) {
sumpp<-function(pp) {
...
lines(theta,tcc(apar[,1],apar[,-1,drop=F],theta),lty=g,col=g)
}
...
for (g in 1:x$ng) {
lines(theta,tcc(gpar[,1,g],matrix(gpar[,-1,g],nrow=ndif),theta),lty=g,col=g)
}
legend("bottomright",labels,lty=1:x$ng,col=1:x$ng,bg="white")
